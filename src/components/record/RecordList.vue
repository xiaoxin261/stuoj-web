<template>
    <ElTable :data="submissions" :key="certinfoKey" style="width: 100%" lazy>
        ...
        <ElTableColumn label="题目">
            <template #default="scope">
                <ProblemName :problem="scope.row.problem" :size="18" />
            </template>
        </ElTableColumn>
        ...
        <ElTableColumn label="语言" width="150">
            <template #default="scope">
                <ElTooltip
                    :content="languages.find((lang: Language) => lang.id === scope.row.language_id)?.name || 'Unknown'">
                    <span class="ellipsis">{{ languages.find((lang: Language) => lang.id ===
                        scope.row.language_id)?.name || 'Unknown' }}</span>
                </ElTooltip>
            </template>
        </ElTableColumn>
        <ElTableColumn label="提交时间" width="155">
            <template #default="scope">
